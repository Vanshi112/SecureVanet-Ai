...
class Trainer:
    ...
    def __init__(self, model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, config: ModelConfig):
        ...
        print('INIT 2')
        try:
            class_weights = torch.tensor(self.config.class_weights, dtype=torch.float32, device=self.device)
            print('PASS 1', flush=True)
            self.criterion = nn.CrossEntropyLoss(weight=class_weights)
            print('PASS 2', flush=True)
            params = list(self.model.parameters())
            print(f'Number of parameter tensors: {len(params)}', flush=True)
            ...
            print('PASS 4', flush=True)
            self.best_accuracy = 0
            self.metrics = Metrics()
            print('PASS 5', flush=True)
            self.early_stopping = EarlyStopping(patience=5, min_delta=0.0001)
            print('PASS 6', flush=True)
            self.checkpoint = CheckpointManager()
            print('PASS 7', flush=True)
            self.writer = SummaryWriter(log_dir='ml/experiments/runs')
            print('PASS 8', flush=True)
            self.scaler = GradScaler('cuda', enabled=torch.cuda.is_available())
            print('PASS 9', flush=True)
    ...
    def save_checkpoint(self, epoch, loss, accuracy):
        self.checkpoint.save(model=self.model, optimizer=self.optimizer, epoch=epoch, loss=loss, accuracy=accuracy, filename='best_model.pth')
    ...
    def train(self):
        ...
        for epoch in range(self.config.epochs):
            ...
            self.checkpoint.save(model=self.model, optimizer=self.optimizer, epoch=epoch + 1, loss=val_loss, accuracy=val_acc, filename='last_model.pth')
